...
export type On = ((
  event: 'update',
  listener: (update: Td.Update) => void
) => Client) &
...
export class Client {
  ...
  private _connectionStateName: Td.ConnectionState['_'] =
  ...
  on: On = (event, fn) => {
    ...
    if (listeners == null) listeners = this._events[event] = new Set();
    listeners.add(fn);
    return this;
  };
  ...
  emit: Emit = (event, value?: any) => {
    ...
    for (const listener of listeners) {
      if (listener.once === true) listeners.delete(listener);
      listener(value);
    }
  };
        ...
        new Promise((resolve, reject) => {
          setTimeout(() => {
            ...
            if (defer != null) {
              finish();
              return reject(
                new Error('Cannot call next() twice in succession')
              );
            }
  ...
  private _finishInit(): void {
    ...
    this._initialized = true;
    for (const r of this._preinitRequests) this._send(r.request, r.id);
  ...
  invoke: Td.Invoke = (request: any): Promise<any> => {
    ...
    this._requestId++;
    if (id >= Number.MAX_SAFE_INTEGER) throw new Error('Too large request id');
  ...
  private _handleClose(): void {
    ...
    if (!this._client.isTdn) this._tdjson.tdold.destroy(this._client.val);
  ...
  private async _loop(): Promise<void> {
    ...
    try {
      while (true) {
        ...
        const responseString = await this._tdjson.tdold.receive(
          this._client.val
        );
  ...
  private _handleReceive(res: any): void {
    ...
    if (defer != null) {
      ...
      if (isError) defer.reject(new TDLibError(res.code, res.message));
  ...
  private _handleUpdate(update: Td.Update): void {
    ...
    switch (update._) {
      case 'updateOption':
        if (
          update.name === 'version' &&
          update.value._ === 'optionValueString'
        ) {
  ...
  login = (arg: LoginDetails | (() => LoginDetails) = {}): Promise<void> => {
    return new Promise((resolve, reject) => {
      ...
      function needLoginDetails(
        l: StrictLoginDetails | null
      ): asserts l is StrictLoginDetails {
      ...
      function needUserLogin(
        l: StrictLoginDetails | null
      ): asserts l is LoginUser {
        if (l == null) needLoginDetails(l);
        if (l.type !== 'user')
          throw new Error(
            'Expected to log in as a bot, received user auth update'
          );
      }
      const processAuthorizationState = async (
        authState: Td.AuthorizationState
      ) => {
        ...
        try {
          switch (authState._) {
            ...
            case 'authorizationStateWaitPassword': {
              ...
              let retry = false;
              while (true) {
                const password = await loginDetails.getPassword(
                  passwordHint,
                  retry
                );
                try {
